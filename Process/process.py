...
def loadBiData(dataname, treeDic, fold_x_train, fold_x_test, TDdroprate):
    data_path = os.path.join(cwd,'data',dataname, dataname + 'graph')
    print("loading train set", )
    traindata_list = BiGraphDataset(fold_x_train, treeDic, tddroprate=TDdroprate,  data_path=data_path)
    print("train no:", len(traindata_list))
    print("loading test set", )
